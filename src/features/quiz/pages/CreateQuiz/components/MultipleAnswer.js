...
const SingleAnswer = ({ question, onChangeQuestion }) => {
  ...
  const onSelectCorrectAnswer = (index) => {
    const newCorrectAnswer = correctAnswer.includes(index.toString())
      ? correctAnswer.filter((item) => item !== index.toString())
      : correctAnswer.concat(index.toString());

    onChangeQuestion({
      ...question,
      correctAnswer: newCorrectAnswer.join("|"),
    });
  };
  ...
  return (
    <div className="answers">
      {question.answers.map((answer, index) => {
        const isCorrect = correctAnswer.includes(index.toString());
        return (
          <div
            key={index}
