...
const columns = [
  ...
  },
  {
    ...
    sorter: true,
    dataIndex: "correctAnswersPercent",
    align: "right",
    render: (correctAnswersPercent) => {
      console.log("correctAnswersPercent", correctAnswersPercent);
      return (
        <>
          <Row gutter={[4, 4]} justify="space-between">
            ...
            <Col span={12}>{correctAnswersPercent}%</Col>
          </Row>
        </>
      );
    },
...
function correctAnswer(user) {
  return (user?.playerAnswers || []).reduce((acc, cur) => {
    const result = user.report.reportQuestions.filter(
      (reportQ) => reportQ.id === cur.questionId
    );
...
const ReportDetailPlayers = ({ report }) => {
  ...
  const fetchData = async () => {
    ...
    const data = await reportApi.getPlayerInReport({
      id,
      offset: 0,
      limit: limit,
    });
        ...
        data.map((user, index) => {
          return {
            ...
            correctAnswersPercent:
              (correctAnswer(user) / user.report.reportQuestions.length) * 100,
